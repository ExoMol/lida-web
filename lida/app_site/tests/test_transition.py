...
class TestTransition(TestCase):
    ...
    def test_str(self):
        tr = Transition.objects.create(
            initial_state=self.state_high, final_state=self.state_low,
    ...
    def test_repr(self):
        tr = Transition.objects.create(
            pk=42, initial_state=self.state_high, final_state=self.state_low,
            partial_lifetime=0.42, branching_ratio=0.42, delta_energy=0.42
        )
    ...
    def test_create_from_data_invalid(self):
        ...
        with self.assertRaises(TransitionError):
            Transition.create_from_data(
                self.diff_state_high, self.state_low, partial_lifetime=0.1,
                branching_ratio=0.1
            )
        with self.assertRaises(TransitionError):
            Transition.create_from_data(
                self.state_high, self.diff_state_low, partial_lifetime=0.1,
                branching_ratio=0.1
            )
        with self.assertRaises(TransitionError):
            Transition.create_from_data(
                self.state_high, self.state_high, partial_lifetime=0.1,
                branching_ratio=0.1
            )
        ...
        _ = Transition.create_from_data(
            self.diff_state_high, self.diff_state_low, partial_lifetime=0.1,
            branching_ratio=0.1
        )
    ...
    def test_delta_energy(self):
        ...
        self.assertEqual(
            t.delta_energy, self.state_low.energy - self.state_high.energy
        )
